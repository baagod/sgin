...
4. **确认先行**:
   - **严禁** 直接执行修改。
   - **必须** 先解释方案，且经过用户明确确认后，方可执行文件写入操作。
...
<INSTRUCTIONS>
1. **Execute the required command** to retrieve the changes: `git_add .`, `git diff --staged -U5 -w > [tmp/code-preview.diff]`.
...
1. **等待用户指令**：等待用户对你明确发出 “编写提交信息” 指令 **（不要擅自提前编写或提交）**。
