...
func Convert(t reflect.Type, field string, v any) any {
	...
	// 新增优化: 如果底层类型兼容 (如 json.RawMessage <-> []byte)，直接一转换。
	if vv.Type().ConvertibleTo(t) {
		return vv.Convert(t).Interface()
	}

	// 2. 【源数据解包】：处理源数据中的指针或接口。
	...
	// 3. 【目标类型包装】：如果目标字段类型是指针（例如 *int）。
	// 策略：先递归转换出底层类型的值（int），然后再为其创建并填充指针。
	// 这种设计可以自然支持多级指针（如 **int）。
	if t.Kind() == reflect.Ptr {
		val := Convert(t.Elem(), field, vv.Interface())
		if val == nil {
			return nil
		}
		// 反射创建指针：相当于 ptr := new(T); *ptr = val
		ptr := reflect.New(t.Elem())
		ptr.Elem().Set(reflect.ValueOf(val))
		return ptr.Interface()
	}

	// 4. 切片递归重组：处理切片（例如 []int 或 [][]int）。
