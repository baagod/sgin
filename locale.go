...
// languageMapping 存储语言标签到翻译器构造函数的映射
var languageMapping = map[language.Tag]struct {
    translator func() locales.Translator
    register   func(*validator.Validate, ut.Translator) error
}{
    language.Chinese:           {zh.New, tzh.RegisterDefaultTranslations},
    language.SimplifiedChinese: {zh_Hans.New, tzh.RegisterDefaultTranslations},
    language.English:           {en.New, ten.RegisterDefaultTranslations},
...
func SupportedLanguages() []language.Tag {
    tags := make([]language.Tag, 0, len(languageMapping))
...
// 返回 language.Matcher, *ut.UniversalTranslator 和默认语言标签
// 如果 validate 为 nil 或 tags 为空，直接返回零值
